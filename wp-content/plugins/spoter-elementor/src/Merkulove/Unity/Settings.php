...
final class Settings {
        ...
        <aside class="mdc-drawer">
            <div class="mdc-drawer__content">
                <nav class="mdc-list">
                    ...
                    <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Plugin settings', 'spoter-elementor' ) ?></h6>
					<?php
					...
					foreach ( $tabs as $tab => $value ) {
						...
						/** Prepare link. */
						$link = admin_url( 'admin.php?page=mdp_spoter_elementor_settings&tab=' . $tab );
	...
	private function support_link() {
	    ...
	    ?>
        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Helpful links', 'spoter-elementor' ) ?></h6>

        <a class="mdc-list-item" href="https://docs.merkulov.design/tag/spoter" target="_blank">
            <i class="material-icons mdc-list-item__graphic" aria-hidden="true">collections_bookmark</i>
            <span class="mdc-list-item__text"><?php echo esc_html__( 'Documentation', 'spoter-elementor' ) ?></span>
        </a>

		<?php if ( TabActivation::get_instance()->is_activated() ) : /** Activated. */ ?>
            <a class="mdc-list-item" href="https://1.envato.market/spoter-elementor-support" target="_blank">
                <i class="material-icons mdc-list-item__graphic" aria-hidden="true">mail</i>
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Get help', 'spoter-elementor' ) ?></span>
            </a>
            <a class="mdc-list-item" href="https://1.envato.market/cc-downloads" target="_blank">
                ...
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Rate this plugin', 'spoter-elementor' ) ?></span>
            </a>
		<?php endif; ?>
        ...
        <a class="mdc-list-item" href="https://1.envato.market/cc-merkulove" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'More plugins', 'spoter-elementor' ) ?></span>
        </a>
		<?php
	...
	private function add_submenu_elementor() {
        ...
        add_submenu_page(
            $parent,
            esc_html__( 'Spoter Settings', 'spoter-elementor' ),
            esc_html__( 'Spoter for Elementor', 'spoter-elementor' ),
            'manage_options',
            'mdp_spoter_elementor_settings',
            [ $this, 'options_page' ]
        );
    ...
    private function render_nag_saved() {
        ...
        /** Render "Settings Saved" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Settings saved!', 'spoter-elementor' ) );
    ...
    private function render_nag_activation() {
        ...
        if ( TabActivation::get_instance()->is_activated() ) {
            ...
            UI::get_instance()->render_snackbar( esc_html__( 'Plugin activated successfully.', 'spoter-elementor' ), 'success', 5500 );
            return;

        }

        /** Render "Activation failed" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Invalid purchase code.', 'spoter-elementor' ), 'error', 5500 );
        ...
        <div class="mdc-drawer__header mdc-plugin-fixed">
            <a class="mdc-list-item mdp-plugin-title" href="#">
                <i class="mdc-list-item__graphic" aria-hidden="true">
                    <img src="<?php echo esc_attr( Plugin::get_url() . 'images/logo-color.svg' ); ?>" alt="<?php esc_html_e( 'SpoterElementor', 'spoter-elementor' ) ?>">
                </i>
                <span class="mdc-list-item__text">
                    <?php if ( 'wordpress' === Plugin::get_type() ) : ?>
                        <?php esc_html_e( 'Spoter for Elementor', 'spoter-elementor' ) ?>
                    <?php else: ?>
                        <?php esc_html_e( 'Spoter', 'spoter-elementor' ) ?>
                    <?php endif; ?>
                </span>
                <span class="mdc-list-item__text">
                    <sup>
                        <?php esc_html_e( 'v.', 'spoter-elementor' ); ?>
            ...
            <button type="submit" name="submit" id="submit" class="mdc-button mdc-button--dense mdc-button--raised">
                <span class="mdc-button__label"><?php esc_html_e( 'Save changes', 'spoter-elementor' ) ?></span>
            </button>
        </div>
		<?php
	...
	public function get_options() {
        ...
        foreach ( $tabs as $tab_slug => $tab ) {
	        ...
	        $opt_name = "mdp_spoter_elementor_{$tab_slug}_settings";
