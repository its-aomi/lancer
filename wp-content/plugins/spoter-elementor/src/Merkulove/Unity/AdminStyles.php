...
final class AdminStyles {
	...
	private function settings_styles() {
		...
		if ( in_array( $screen->base, Plugin::get_menu_bases(), true ) ) {
			...
			wp_enqueue_style( 'mdp-spoter-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/merkulov-ui.min.css', [], Plugin::get_version() );
            wp_enqueue_style( 'mdp-spoter-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
			wp_enqueue_style( 'mdp-spoter-elementor-admin', Plugin::get_url() . 'css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
	...
	private function plugin_update_styles() {
		...
		if ( isset( $_GET['plugin'] ) && $_GET['plugin'] === 'spoter-elementor' ) {

			wp_enqueue_style( 'mdp-spoter-elementor-plugin-install', Plugin::get_url() . 'src/Merkulove/Unity/assets/css/plugin-install' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
