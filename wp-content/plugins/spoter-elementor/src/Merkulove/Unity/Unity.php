...
final class Unity {
    ...
    public static function restore_settings() {
    	...
    	if ( is_array( get_option( 'mdp_spoter_elementor_general_settings' ) ) ) { return; }

    	/** Try to get old options */
	    $old_general_options = get_option( 'mdp_spoter_elementor_settings' );
	    $old_css_options = get_option( 'mdp_spoter_elementor_css_settings' );

	    /** Restore General and remove old General settings */
	    if ( is_array( $old_general_options ) ) {

		    update_option( 'mdp_spoter_elementor_general_settings', $old_general_options, true );
		    delete_option( 'mdp_spoter_elementor_settings' );
	    ...
	    if ( is_array( $old_css_options ) ) {
		    ...
		    update_option( 'mdp_spoter_elementor_custom_css_settings', $old_css_options, true );
		    delete_option( 'mdp_spoter_elementor_css_settings' );
	...
	private static function send_install_action() {
		...
		$opt_name = 'mdp_spoter_elementor_send_action_install';
		...
		if ( ! $installed_version ) {
            ...
            /** Send install Action to our host. */
			Helper::get_instance()->send_action( 'install', 'spoter-elementor', Plugin::get_version() );
