...
final class TabMigration extends Tab {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_export_settings_spoter_elementor', [ __CLASS__, 'ajax_export_settings' ] );
        add_action( 'wp_ajax_import_settings_spoter_elementor', [ __CLASS__, 'ajax_import_settings' ] );
	...
	public function add_settings() {
        ...
        $group = 'SpoterElementor' . self::TAB_SLUG . 'OptionsGroup';
        $section = 'mdp_spoter_elementor_' . self::TAB_SLUG . '_page_status_section';

        // Add migration controls only for general plugins
        if ( ! in_array( Plugin::get_type(), [ 'elementor', 'wpbakery', 'divi' ] ) ) {
            add_settings_field( 'import_settings', esc_html__( 'Import settings:', 'spoter-elementor' ), [$this, 'render_import_settings'], $group, $section );
            add_settings_field( 'export_settings', esc_html__( 'Export settings:', 'spoter-elementor' ), [$this, 'render_export_settings'], $group, $section );
        }
		add_settings_field( 'delete_plugin', esc_html__( 'Removal settings:', 'spoter-elementor' ), [$this, 'render_delete_plugin'], $group, $section );
	...
	public function render_delete_plugin() {
		...
		/** Prepare description. */
		$helper_text = esc_html__( 'Choose which data to delete upon using the "Delete" action in the "Plugins" admin page.', 'spoter-elementor' );

		/** Render select. */
		UI::get_instance()->render_select(
			$options,
			$uninstall_settings[ 'delete_plugin' ], // Selected option.
			esc_html__( 'Delete plugin', 'spoter-elementor' ),
			$helper_text,
			[
			    'name' => 'mdp_spoter_elementor_migration_settings[delete_plugin]'
            ]
		);
    ...
    public function render_import_settings() {
        ...
        UI::get_instance()->render_import(
            '',
            esc_html__( 'Import settings', 'spoter-elementor' ),
            esc_html__( 'Drag & Drop file here', 'spoter-elementor' ),
            'import_settings',
            'migration'
        );
    ...
    public function render_export_settings() {
        ...
        foreach ( $tabs as $slug => $tab ) {
            ...
            UI::get_instance()->render_switcher(
                ...
                $tab['label'] . ' ' . esc_html__( 'Tab', 'spoter-elementor' ),
                esc_html__( 'Export all settings from', 'spoter-elementor' ) . ' ' .  $tab['label'] . ' ' . esc_html__( 'tab', 'spoter-elementor' ),
                [
                    "name" => wp_sprintf(
                        'mdp_spoter_elementor_%s_settings[export_%s]',
                        self::TAB_SLUG,
                        $slug
                    )
                ]
        ...
        UI::get_instance()->render_button(
            esc_html__( 'Export settings', 'spoter-elementor' ),
            '',
            'file_download',
            [
                "name" => 'mdp_spoter_elementor_' . self::TAB_SLUG . '_settings' . "[export_selected]",
                "id" => "mdp-export-btn",
    ...
    public function ajax_export_settings() {
        ...
        check_ajax_referer( 'spoter-elementor', 'nonce' );
        ...
        foreach ( $options as $opt => $val ) {
            ...
            if ( strpos( $opt,'export_' ) === 0 && $val === 'on' ) {
                ...
                $export[ $tab ] = get_option( 'mdp_spoter_elementor_' . $tab . '_settings' );
            }
    ...
    public function ajax_import_settings() {
        ...
        // Get settings
        $import = $_POST[ 'import' ] ? $_POST[ 'import' ] : '';
        ...
        foreach ( $settings as $tab => $set ) {
            ...
            if ( $set === false ) {
                ...
                // Reset to default
                delete_option( 'mdp_spoter_elementor_' . $tab . '_settings' );

            } else {

                // Update settings
                update_option( 'mdp_spoter_elementor_' . $tab . '_settings', $set );
    ...
    private function import_errors( $e, $plugin = array() ) {
        ...
        switch ( $e ) {
            ...
            case 1:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'No settings for import', 'spoter-elementor' )
                );
                break;
            case 2:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Wrong settings format', 'spoter-elementor' )
                );
                break;
            case 3:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for', 'spoter-elementor' ) . ' ' . ucwords( $plugin[ 'slug' ] ) . esc_html__( ', but this is', 'spoter-elementor' ) . ' ' . Plugin::get_name()
                );
                break;
            case 4:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'You are importing settings for version', 'spoter-elementor' ) . ' ' . ucwords( $plugin[ 'version' ] ) . ', but the current version is' . ' ' . Plugin::get_version()
                );
                break;
            case 5:
                $message = array(
                    'status' => true,
                    'message' => esc_html__( 'Settings imported', 'spoter-elementor' )
                );
                break;
            default:
                $message = array(
                    'status' => false,
                    'message' => esc_html__( 'Unknown error', 'spoter-elementor' )
                );
