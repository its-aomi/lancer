...
final class PluginUpdater {
     ...
     * @var string
	 **/
	private $opt_plugin_info = 'mdp_spoter_elementor_plugin_info';
	...
	private function __construct() {
		...
		add_action( 'in_plugin_update_message-spoter-elementor/spoter-elementor.php', [$this, 'show_plugin_update_message' ], 10, 2 );
	...
	private function compare_with_new( $plugin_info, $wp_version, $update_plugins ) {
		...
		if ( version_compare( $latest_version, $current_version ) > 0 ) {
			...
			$update_plugins->response['spoter-elementor/spoter-elementor.php'] = (object)[
				'slug' => 'spoter-elementor',
                'plugin' => 'spoter-elementor/spoter-elementor.php',
	...
	public function plugin_info( $res, $action, $args ) {
		...
		if ( 'spoter-elementor' !== $args->slug ) { return false; }
	...
	private function get_plugin_information() {
        ...
        if ( ! is_array( $plugin_info ) ) {
	        ...
	        if ( ! $plugin_info->mdp_spoter_elementor_plugin_info ) {
		        return false;
	        } else {
		        $plugin_info = json_decode( $plugin_info->mdp_spoter_elementor_plugin_info->body, false );
	        }
        }
	...
	public function show_plugin_update_message( $plugin_data, $r ) {
		...
		if ( ! TabActivation::get_instance()->is_activated() ) {
            ...
            esc_attr_e( 'Your plugin license is ', 'spoter-elementor' );
            echo '<strong style="color: #ff495c">';
            esc_attr_e( 'not activated', 'spoter-elementor' );
            echo '.</strong> ';
            esc_attr_e( 'Please activate the license to receive automatic updates in one click, or visit the plugin page on the Envato market to ', 'spoter-elementor' );
            echo ' <a href="https://1.envato.market/reviewer-elementor" target="_blank">';
			echo ' <a href="https://1.envato.market/spoter-elementor" target="_blank">';
			esc_attr_e( 'download ', 'spoter-elementor' );
			echo '</a> ';
			esc_attr_e( 'the latest version.', 'spoter-elementor' );
		}
