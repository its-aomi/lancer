...
final class Rest {
	...
	private function __construct() {
		...
		add_action( 'rest_api_init', function () {
			...
			register_rest_route(
				...
				untrailingslashit( 'spoter-elementor/v2' ),
				'/(?P<action>\w+)/',
				array(
	...
	public function callback( $params ) {
		...
		// Prepare url
		$url = wp_sprintf(
			'https://merkulove.host/wp-json/mdp/v2/%s?plugin=spoter-elementor&name=%s&mail=%s&domain=%s',
			$action,
			$params[ 'name' ] ?? '',
			$params[ 'mail' ] ?? '',
            $this->clear_url()
	...
	private function get_ssl_args() {
		...
		return [
			'timeout'    => 30,
			'user-agent' => 'spoter-elementor-user-agent',
