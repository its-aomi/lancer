...
final class AdminScripts {
	...
	private function settings_scripts() {
        ...
        wp_enqueue_script( 'mdp-spoter-elementor-ui', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/merkulov-ui' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );

        /** Add tab related scripts */
        if ( isset( $_GET[ 'tab' ] ) && $_GET[ 'tab' ] === 'migration' ) {
            wp_enqueue_script( 'mdp-spoter-elementor-unity-migration-tab', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/migration' . Plugin::get_suffix() . '.js', [], Plugin::get_version(), true );
        }
        ...
        $to_js = [
            ...
            'nonce'     => wp_create_nonce( 'spoter-elementor' ), // Nonce for security.
        ];

        wp_enqueue_script( 'mdp-spoter-elementor-unity-admin', Plugin::get_url() . 'src/Merkulove/Unity/assets/js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
        wp_localize_script( 'mdp-spoter-elementor-unity-admin', 'mdpSpoterElementor', $to_js );
        wp_enqueue_script( 'mdp-spoter-elementor-admin', Plugin::get_url() . 'js/admin' . Plugin::get_suffix() . '.js', [ 'jquery' ], Plugin::get_version(), true );
